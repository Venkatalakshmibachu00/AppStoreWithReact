...
class AppStore extends Component {
  state = {activeTabId: tabsList[0].tabId, seachInput: ''}

  displayApps = id => {
    this.setState({activeTabId: id})
  }

  onChangeSeachInput = event => this.setState({seachInput: event.target.value})

  render() {
    const {activeTabId, seachInput} = this.state

    const filterdApps = appsList.filter(
      eachItem => eachItem.category === activeTabId,
    )

    const searchAppList = filterdApps.filter(eachSeach =>
      eachSeach.appName
        .toLocaleLowerCase()
        .includes(seachInput.toLocaleLowerCase()),
    )
        ...
        <ul className="tab-item-list-container">
          {tabsList.map(eachTab => (
            <TabItem
              key={eachTab.tabId}
              eachTab={eachTab}
              displayApps={this.displayApps}
              isActive={activeTabId === eachTab.tabId}
            />
          ))}
        </ul>
        <ul className="app-item-list-container">
          {searchAppList.map(eachApp => (
